...
function getTeamStats(teamId) {
  ...
  $.getJSON(
    'https://lookup-service-prod.mlb.com/json/named.roster_40.bam?team_id=' +
      teamId +
      '&roster_40.col_in=position_txt,name_display_first_last,jersey_number,bats,throws,player_id',
    function(players) {
      //console.log(players.roster_40.queryResults.row);
      for (var i in players.roster_40.queryResults.row) {
        teamMembers[i] = new Player(
          players.roster_40.queryResults.row[i].name_display_first_last,
      ...
      function lookupPitcherStats(pitcher) {
        ...
        $.getJSON(
          "https://lookup-service-prod.mlb.com/json/named.sport_pitching_tm.bam?league_list_id='mlb'&game_type='R'&season=" +
            season +
            "&teamId='119'&player_id=" +
            pitcher.playerId +
            '&sport_pitching_tm.col_in=g,era,gs,w,l,ip',
          function(pStats) {
            //console.log(pStats);
            if (pStats.sport_pitching_tm.queryResults.row == undefined) {
              ...
              stats.era =
                pStats.sport_pitching_tm.queryResults.row[
                  pStats.sport_pitching_tm.queryResults.row.length - 1
                ].era;
              stats.games =
                pStats.sport_pitching_tm.queryResults.row[
                  pStats.sport_pitching_tm.queryResults.row.length - 1
              ...
              stats.inningsPitched =
                pStats.sport_pitching_tm.queryResults.row.ip;
      ...
      function lookupHitterStats(hitter) {
        ...
        $.getJSON(
          "https://lookup-service-prod.mlb.com/json/named.sport_hitting_tm.bam?league_list_id='mlb'&game_type='R'&season=" +
            season +
            "&teamId='119'&player_id=" +
            hitter.playerId +
            '&sport_hitting_tm.col_in=g,avg,ab,rbi,hr,bb,so',
          function(hStats) {
            //console.log(pStats);
            if (hStats.sport_hitting_tm.queryResults.row == undefined) {
              ...
              stats.games =
                hStats.sport_hitting_tm.queryResults.row[
                  hStats.sport_hitting_tm.queryResults.row.length - 1
                ].g;
              stats.average =
                hStats.sport_hitting_tm.queryResults.row[
                  hStats.sport_hitting_tm.queryResults.row.length - 1
      ...
      function setPitcherStats(pitcherStats, pitcher) {
        ...
        addPitcher(
          ...
          pitcher.stats.games,
          pitcher.stats.era,
          pitcher.stats.gamesStarted,
          pitcher.stats.inningsPitched,
          pitcher.stats.wins,
          pitcher.stats.losses
        );
      ...
      function setHitterStats(hitterStats, hitter) {
        ...
        addHitter(
          hitter.name,
          hitter.stats.games,
          hitter.stats.average,
          hitter.stats.atBats,
          hitter.stats.rbi,
          hitter.stats.homeruns,
          hitter.stats.strikeOuts
        );
